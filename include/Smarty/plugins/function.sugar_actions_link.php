...
function smarty_function_sugar_actions_link($params, &$smarty)
{
    ...
    if (!is_array($type)) {
        ...
        switch (strtoupper($type)) {
            ...
            case "CARO_EXPORT":
                return '{if $bean->aclAccess("export")}<a title="{$APP.LBL_EXPORT}" accessKey="e" onclick="" href="caro_export_template(\''. $module .'\', \'{$id}\')" name="caro_export" id="caro_export_button">{$APP.LBL_EXPORT}</a>{/if}';
                break;
