...
function smarty_function_sugar_button($params, &$smarty)
{
    ...
    if (!is_array($type)) {
        ...
        switch (strtoupper($type)) {
            ...
            case "CARO_EXPORT":
                $output = '{if $bean->aclAccess("export")}<input title="{$APP.LBL_EXPORT}" accessKey="e" class="button" onclick="caro_export_template(\''. $module .'\', \'{$id}\')" type="button" name="caro_export" value="{$APP.LBL_EXPORT}" id="caro_export_button" />{/if}';
                break;
